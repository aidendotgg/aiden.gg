...
import type { DiscordPresence, Tech } from '@aiden/types';
...
export default function About() {
  ...
  useEffect(() => {
    const socket = new WebSocket(`${process.env.BACKEND_URL?.startsWith('https') ? 'wss' : 'ws'}://${process.env.BACKEND_URL?.replace('http://', '').replace('https://', '')}/presence`);
    ...
    <>
      <section id="about" className="max-w-4xl w-full flex flex-col mx-auto">
        ...
        <ul className="grid grid-cols-2 gap-4 mt-4">
          ...
          />
          {presence && presence.activities.length > 0 && <PresenceCard presence={presence} date={date} direction="bottom" span={1} delay={0.1} gradient="bg-gradient-to-tl" />}
        </ul>
      </section>
    </>
  );
