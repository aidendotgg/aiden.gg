...
export default function ExperienceCard({
  ...
  return (
    <>
      ...
      >
        <div className={`p-4 flex md:flex-row flex-col gap-6 ${gradient} from-primary to-secondary rounded-lg border border-accent shadow-2xl shadow-background items-center`}>
          <img alt="" draggable={false} className="rounded-lg md:h-56 sm:h-48 h-40 duration-300" src={cardImage} />
          <div className="flex flex-col">
            <h2 className="md:text-left text-center font-semibold text-4xl">{myRole}</h2>
            ...
            <div className="flex row gap-4 mt-2">
              {url && <LinkButton label="Visit Website" link={url} width="w-1/2" />}
            ...
            <div className="flex lg:flex-row flex-col justify-between mt-6 px-3">
              <div className="flex flex-col">
                <div className="flex flex-row gap-2 items-center">
                  ...
                  {url && (
                    <Link href={url} target="_blank" className="bg-middle hover:bg-secondary duration-300 border border-accent p-1.5 rounded-full">
              ...
              <div className="flex flex-col">
                <h2 className="sm:text-2xl text-xl font-semibold">Overview</h2>
                <div className="max-h-66 overflow-y-auto bg-neutral-800 border border-accent rounded-lg p-2">
                  {fullDescription.map((desc, i) => (
                    <p key={i} className="sm:text-lg text-base first:mt-0 mt-2 max-w-md">
                      {desc}
                    </p>
                  ))}
                </div>
