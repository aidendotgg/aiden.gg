...
export default function PresenceCard({
  ...
  return (
    <>
      ...
      >
        <div className={`${gradient} from-primary to-secondary p-4 flex flex-col rounded-lg border border-accent shadow-2xl shadow-background`}>
          <style jsx global>
            {`
              .control-dots {
                display: none;
          ...
          <h2 className="text-center font-semibold text-4xl">Currently Doing</h2>
          <p className="text-center text-xl mb-1.5">Below are the activities I am currently doing.</p>
          <Divider />
          <Carousel className="w-full rounded-lg mt-2" showArrows={false} showStatus={false} autoPlay={true} stopOnHover={true} infiniteLoop={true} emulateTouch={true} showThumbs={false}>
              ...
              <div key={index} className="flex min-[450px]:flex-row flex-col gap-4 items-center px-1 select-none">
                ...
                {activity.name === 'Spotify' ? (
                  <div className="flex flex-col overflow-x-hidden w-full min-[450px]:text-left text-center">
                    <h1 className="text-lg font-bold leading-7">{activity.title}</h1>
                    {activity.details && <p className="text-lg font-medium leading-6 text-nowrap truncate">{activity.details}</p>}
                    {activity.state && <p className="text-lg font-medium leading-6 text-nowrap truncate">by {activity.state}</p>}
                  ...
                  <div className="flex flex-col overflow-x-hidden w-full min-[450px]:text-left text-center">
                    ...
                    {activity.timestamps?.end && !activity.timestamps?.start && (
                      <p className="text-base leading-6 text-nowrap truncate">{new Date(new Date(activity.timestamps?.end!).getTime() - date.getTime()).toISOString().slice(11, 19)} left</p>
                    )}
                    {activity.timestamps?.start && !activity.timestamps?.end && (
                      <p className="text-base leading-6 text-nowrap truncate">{new Date(date.getTime() - new Date(activity.timestamps?.start!).getTime()).toISOString().slice(11, 19)} elapsed</p>
                    )}
                  </div>
                )}
              </div>
